# Design and implementation detail — `isr-intelligence-model`

This document is the **authoritative design note** for the **as-built** Java domain module under `isr-intelligence-modules/isr-intelligence-model` (field names, semantics, and toolchain). The **`com.isr.intelligence.model`** types and their Javadoc are the executable contract. Entity types are **mutable JavaBeans** (not records) so the same instance can be updated incrementally and held in a cache without rebuilding the whole aggregate on each change.
...
- Use **Java 21** and **Maven 3.9.x** from the repo toolchain ([`isr-tools/env.ps1`](../../isr-tools/env.ps1); JDK/Maven under `D:\isr\isr-tools\.tools\`).

## Maven coordinates and build

| Item | Value |
|------|--------|
| `groupId` | `com.isr.intelligence` |
| `artifactId` | `isr-intelligence-model` |
| `version` | `0.1.0` |
| `packaging` | `jar` |
| Parent POM | None (standalone module, same style as `isr-intelligence-solr-server`) |
...
Java sources live in **`com.isr.intelligence.model`**:

```
src/main/java/com/isr/intelligence/model/
├── AttributeType.java
...
- **Java** uses the names in this document and in the classes under `com.isr.intelligence.model` (camelCase with getters/setters).
- The web domain type may still use `tags` for keywords; aligning TS/OpenAPI is **out of scope** for the JAR itself.

## Verification

From the repo root (after `.\isr-tools\env.ps1` on Windows so `JAVA_HOME` is JDK 21):

```powershell
mvn -f isr-intelligence-modules/isr-intelligence-model/pom.xml package
```
