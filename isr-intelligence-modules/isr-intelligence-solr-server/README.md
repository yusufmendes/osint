...
- **Java 21** (example: isolated toolchain from repo root `isr-tools`)
...
```powershell
cd D:\isr\isr-intelligence-modules\isr-intelligence-solr-server
...
Default image name: `isr-intelligence-solr-server:0.1.0` (aligned with `docker.image.name` and `project.version`).

Run:

```powershell
docker run -p 8983:8983 isr-intelligence-solr-server:0.1.0
```

Inside the container, `run-app.sh` runs the credential jar with the **Java 21 JRE added by the Dockerfile** (`JAVA_CREDENTIALS`, default `/opt/isr/jre-21/bin/java`); Solr itself starts via `solr-foreground` using the official image JVM. Details: [DesignAndImplementationDetail.md](DesignAndImplementationDetail.md).
...
| `docker.image.name` | `isr-intelligence-solr-server` | Docker image name |

## Frontend alignment

`isr-intelligence-web` default Solr URL:
