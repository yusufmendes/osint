# isr-intelligence-server

Spring Boot service that exposes the isr intelligence platform as a REST API. PostgreSQL/PostGIS is the system of record; Apache Solr is kept eventually consistent through a transactional outbox.
...
```
┌───────────┐  HTTP/JSON  ┌───────────────────────────┐
│  Client   │ ──────────▶ │  isr-intelligence-server │
...
| jOOQ | 3.19 (via `spring-boot-starter-jooq`) | Type-safe SQL DSL — we hand-write the table refs in [`Tables.java`](src/main/java/com/isr/intelligence/server/db/Tables.java) instead of running codegen at build time, so the build does not need a live database |
...
3. **The shared model module installed locally**, so the server can resolve `isr-intelligence-model`:

   ```powershell
   mvn -f .\isr-intelligence-modules\isr-intelligence-model\pom.xml -DskipTests install
   ```
...
Solr is provided by the sibling module [`isr-intelligence-solr-server`](../isr-intelligence-solr-server). Follow its README to either build a Docker image (`-Pdeployment`) or start Solr locally with the configset under `src/main/resources/conf`. The server expects a Solr collection named `intelligence` exposed at `http://localhost:8983/solr` by default.

### Step 3 — Build and run the server

```powershell
mvn -f .\isr-intelligence-modules\isr-intelligence-server\pom.xml -DskipTests package
java -jar .\isr-intelligence-modules\isr-intelligence-server\target\isr-intelligence-server.jar
```
...
The Java domain types live in the sibling module [`isr-intelligence-model`](../isr-intelligence-model). They are plain mutable beans (`Intelligence`, `Template`, `Attribute`, `AttributeTypeValue`). The server module wraps them in immutable `record` DTOs (`IntelligenceDto`, `TemplateDto`, …) for transport and persistence.
...
We deliberately do **not** run jOOQ codegen at build time (so the build does not need a running Postgres). Instead, [`Tables.java`](src/main/java/com/isr/intelligence/server/db/Tables.java) hand-writes the table and field references using the same naming jOOQ codegen would produce:
...
```powershell
mvn -f .\isr-intelligence-modules\isr-intelligence-server\pom.xml -Pjooq-codegen generate-sources -DskipTests
```

Generated types land under `target/generated-sources/jooq` in package `com.isr.intelligence.db.generated`. The hand-written constants in `Tables.java` use the same names, so you can migrate repositories to the generated `Tables` incrementally.
...
```powershell
mvn -f .\isr-intelligence-modules\isr-intelligence-server\pom.xml test
```

### 2. End-to-end REST integration tests — `mvn verify`

Classes under `src/test/java/com/isr/intelligence/server/e2e/*IT.java` run in the `integration-test` phase. They:

- Bring up Postgres + PostGIS (`postgis/postgis:16-3.4`) in Docker via Testcontainers.
- Bring up Solr 9 (`solr:9.10.1`) in Docker, preloaded with the `intelligence` core configset copied from the sibling [`isr-intelligence-solr-server`](../isr-intelligence-solr-server/src/main/resources/conf) module.
...
```powershell
mvn -f .\isr-intelligence-modules\isr-intelligence-server\pom.xml verify
```
...
```powershell
mvn -f .\isr-intelligence-modules\isr-intelligence-server\pom.xml -Dit.test=SearchE2EIT verify
```
...
└── src/
    ├── main/
    │   ├── java/com/isr/intelligence/server/
