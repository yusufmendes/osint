...
Standard getters/setters; no-arg and all-args constructors. Fields: **`id`**, **`version`**, **`value`**, **`attributeId`**.
...
| `attributeValueTypeIdList` | `List<String>` | Live **mutable** list (backing **`ArrayList`**); ids of **AttributeTypeValue** rows — used **only** for **ENUM** and **ENUM_LIST** attributes |
...
- Concrete values on an intelligence instance still use **`Intelligence.getAttributeIdToAttributeValueMap()`** (or persistence), depending on the service layer.

**`setAttributeValueTypeIdList`** replaces list contents; **`getAttributeValueTypeIdList()`** returns the live list for **`add`** / **`clear`**. Helpers **`addAttributeValueTypeId`** / **`addAllAttributeValueTypeIds`** are provided.
...
**Lists:** backing **`ArrayList`**s; **`set…List`** replaces contents from a copy of the argument (or clears on **`null`**).
...
**Caching:** the module does not implement a cache; use your cache library keyed by entity **`id`** (or hold references) and mutate the same bean instance as needed. For concurrent access, synchronize or copy out of the cache.

## Cross-stack naming

- **Java** uses the names in this document and in the classes under **`com.osint.intelligence.model`** (e.g. **`keywords`**, **`attachedFileUniqueIdList`**, camelCase property names with getters/setters).
